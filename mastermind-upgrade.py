...
def finishedCondition (score, winCount, win):
    ...
    contd = Rectangle (Point (23.3, 26.25), Point (27.7, 28.75))
    contdTxt = Text (Point (27.5, 27.5), "Contiune?")
    contd.draw (win)
    contdTxt.draw (win)

    win.getMouse ()
    runGame (winCount, win)
...
def runGame (winCount, gameWin):
    ...
    # submit = submitButton(gameWin)<----------NEEDS WORK
        ...
        if bPeg == 4:
            ...
            winCount = finishedCondition ('victory', winCount, gameWin)
            break
        elif turnCount == 7:
            hideMaster.undraw()
            revealSolution (list (colorMasterList), gameWin)
            winCount = finishedCondition ('lost', winCount, gameWin)
            break
        else:
